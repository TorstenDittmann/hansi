...
<MarketingShell configured={data.configured}>
	<section class="relative pt-12 pb-10 sm:pt-20 sm:pb-14 lg:flex lg:min-h-[500px] lg:items-center">
		...
		<AsciiHansi
			class="mt-8 w-full text-[6.5px] sm:mt-10 sm:text-[7px] lg:absolute lg:inset-x-0 lg:top-1/2 lg:mt-0 lg:-translate-y-1/2 lg:text-[9px]"
		/>
	</section>
				...
				<div class="min-w-0 flex-1">
					...
					>
						...
						>
							...
							<span><b class="text-stone-900 dark:text-stone-100">2</b> findings filtered out</span>
						</div>
					</div>
				</div>
			</div>
			...
			>
				...
				>
					...
					<div class="min-w-0">
						...
						>
							{@render codeRows([{ sign: '+', text: '  const order = await db.orders.find(id);' }])}
						</div>
					</div>
				</div>
	...
	>
		<div class="min-w-0">
			...
			<p class="mt-2.5 text-stone-500 dark:text-stone-400">
				Hansi flags the bugs you would want a teammate to catch: a missing await, an inverted check,
				an off-by-one. A second pass drops anything it can't confirm.
			</p>
			<div
		...
		<div class="min-w-0">
			...
			<p class="mt-2.5 text-stone-500 dark:text-stone-400">
				Clean pull requests get approved. When something is wrong, Hansi requests changes, and once
		...
		<div class="min-w-0">
			...
			<p class="mt-2.5 text-stone-500 dark:text-stone-400">
				Each review ends with a grade from S, ready to merge, to F, do not merge. Open findings cap
				the grade, so it always matches what Hansi found.
			</p>
			<div
