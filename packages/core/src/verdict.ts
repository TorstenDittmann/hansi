 ...
 * - new blocking findings → request changes (or only comment when `request_changes: never`)
...
export function decideVerdict({ posted, stillOpen, config }: VerdictInput): Verdict {
	...
	if (blocking) return config.reviews.request_changes === 'never' ? 'comment' : 'request_changes';
